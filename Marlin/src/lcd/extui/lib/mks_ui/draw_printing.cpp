  ...
  switch (obj->mks_obj_id) {
    case ID_PAUSE:
      if (uiCfg.print_state == WORKING) {
        ...
        #endif
        lv_imgbtn_set_src_both(buttonPause, "F:/bmp_resume_small.bin");
        lv_label_set_text(labelPause, printing_menu.resume);
        lv_obj_align(labelPause, buttonPause, LV_ALIGN_CENTER, 0, 0);
      }
      else if (uiCfg.print_state == PAUSED) {
        uiCfg.print_state = RESUMING;
        lv_imgbtn_set_src_both(obj, "F:/bmp_pause_small.bin");
...
void lv_draw_printing(void) {
  ...
  buttonPause = lv_imgbtn_create(scr, uiCfg.print_state == WORKING ? "F:/bmp_pause_small.bin" : "F:/bmp_resume_small.bin", 5, 240, event_handler, ID_PAUSE);
  buttonStop = lv_imgbtn_create(scr, "F:/bmp_stop_small.bin", 165, 240, event_handler, ID_STOP);
  buttonOperat = lv_imgbtn_create(scr, "F:/bmp_operate_small.bin", 325, 240, event_handler, ID_OPTION);
  ...
  labelExt1 = lv_label_create(scr, 255, 146, nullptr);
  ...
  #if HAS_HEATED_BED
    labelBed = lv_label_create(scr, 255, 196, nullptr);
  #endif

  labelFan = lv_label_create(scr, 395, 196, nullptr);
  labelTime = lv_label_create(scr, 255, 96, nullptr);
  ...
  if (gCfgItems.multiple_language) {
    ...
    lv_obj_align(labelStop, buttonStop, LV_ALIGN_CENTER, 0, 0);

    lv_label_set_text(labelOperat, printing_menu.option);
    lv_obj_align(labelOperat, buttonOperat, LV_ALIGN_CENTER, 0, 0);
  }
